...
const loginValidationSchema = yup
  ...
  .shape({
    phoneNumber: yup
      ...
      .nullable()
      .notRequired()
      .matches(/^\d+$/, 'phoneNumber must be digit')
...
const otpValidationSchema = yup.object().shape({
  otp: yup
    .string()
    .length(6, 'Otp must be exacty 6 digits')
    .required('otp is required'),
})

const profileValidationSchema = yup.object().shape({
  username: yup.string().required('username is required'),
  agreed: yup.bool().oneOf([true], 'you must agreed to terms'),
})
...
const Login = () => {
    ...
    try {
      ...
      if (response.status === 'success') {
        ...
        if (user?.username && user?.profilePicture) {
          setUser(user)
          toast.success('Welcome back to chatBlizzz')
          navigate('/')
          resetLoginState()
        } else {
          setStep(3)
  ...
  const onProfileSubmit = async (data) => {
    try {
      ...
      toast.success('Welcome back to ChatBlizz')
      navigate('/')
      resetLoginState()
    } catch (error) {
      console.error(error)
  ...
  const ProgressBar = () => (
    <div
      className={`w-full ${theme === 'dark' ? 'bg-grey-700' : 'bg-grey-200'} rounded-full h-2.5 mb-6`}
    >
      <div
  ...
  return (
    <div
      className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gradient-to-br from-pink-200 to-purple-600'} flex items-center justify-center p-4 overflow-hidden`}
    >
      <motion.div
      ...
      >
        <motion.div
          ...
          className="w-24 h-24 bg-purple-500 rounded-full mx-auto mb-6 flex items-center justify-center "
        >
          <IoLogoSnapchat className="w-16 h-16 text-white" />
        </motion.div>
        <h1
            ...
            <div className="relative">
              ...
              <div className="flex">
                <div className="relative w-1/3">
                  <button
                    type="button"
                    className={`shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center ${theme === 'dark' ? 'text-white bg-gray-600' : 'text-gray-900 bg-gray-100 border-gray-300'} border rounde-s-lg hover:bg-gray-200 focus:right-4 focus:outline-none focus:ring-gray-100`}
                  ...
                  {showDropdown && (
                    <div
                      className={`absolute z-10 w-full mt-1 ${theme === 'dark' ? 'border-gray-600 bg-gray-700' : ' bg-white border-gray-300'} border rounded-md shadow-lg max-h-60 overflow-auto`}
                    >
                      <div
                      ...
                      >
                        <input
                          ...
                          className={`w-full px-2 py-1 border ${theme === 'dark' ? 'bg-gray-600 border-gray-500 text-white' : 'bg-white border-gray-300'} rounded-md text-sm focus:outline-none focus:right-2 focus:ring-purple-500 `}
                        />
                      </div>
        ...
        {step === 2 && (
          <form onSubmit={handleOtpSubmit(onOtpSubmit)} className="space-y-4">
            ...
            >
              Please enter the six digit Otp send to your{' '}
              {userPhoneData?.email ||
                `${userPhoneData?.phoneSuffix || ''} ${userPhoneData?.phoneNumber || ''}`.trim()}
            </p>
            ...
            <div className="flex justify-between">
              {otp.map((digit, index) => (
                <input
                  ...
                  className={`w-12 text-center border ${theme === 'dark' ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'} focus:rounded-md focus:outline-none  focus:ring-purple-500 ${otpErrors.otp ? 'border-red-500' : ''}`}
                />
              ))}
            </div>
            {otpErrors.otp && (
            ...
            <button
              type="button"
              onClick={handleBack}
              className={`w-full ${theme === 'dark' ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-700'} py-2 rounded-md hover:bg-gray-300 transition flex items-center justify-center`}
            >
        ...
        {step === 3 && (
          ...
          >
            <div className="flex flex-col items-center mb-4">
              <div className="relative w-24 h-24 mb-2">
                ...
                <label
                  ...
                  className="absolute bottom-0 right-0 bg-purple-500 text-white p-2 rounded-full cursor-pointer hover:bg-purple-600 transition duration-300"
                >
              ...
              <p
                className={`text-sm ${theme === 'dark' ? 'text-grey-300' : 'text-grey-500'} mb-2`}
              >
                Choose Avatar
              </p>
              <div className="flex flex-wrap justify-center gap-2">
                {avatars.map((avatar, index) => (
                  <img
                    ...
                    src={avatar}
                    alt={`Avatar ${index + 1}`}
                    className={`w-12 h-12 rounded-full curser-pointer transition duration-300 ease-in-out transform hover:scale-110 ${selectedAvatar === avatar ? 'ring-2 ring-purple-500' : ''}`}
            ...
            <button
              ...
              className={`w-full bg-purple-500 text-white font-bold px-4 py-3 rounded-md hover:scale-105 transition duration-300 ease-in-out transform flex items-center justify-center text-lg
